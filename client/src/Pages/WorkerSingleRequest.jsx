...
const WorkerSingleRequest = () => {
  ...
  const cancel_req=async ()=>{
    try{
      const res=await axios.post("http://localhost:8000/yash-services/services/worker-cancel-req",{hid:request._id})
      console.log(res.data)
    }
    catch(err){
      console.log(err)
      ...
      <div className="bg-green-200 w-[800px] h-[620px] mx-auto mt-7 rounded-3xl p-10">
        ...
        <div className="font-mono w-[500px] mx-auto h-[500px] text-xl">
          ...
          <div className="ml-10 mt-10 flex flex-row gap-8">
            <button className="bg-green-500 text-white p-3 rounded-3xl text-center font-mono hover:cursor-pointer hover:opacity-75" onClick={accept_req}>
              Accept
            </button>
            <button className="bg-red-500 text-white p-3 rounded-3xl text-center font-mono hover:cursor-pointer hover:opacity-75" onClick={cancel_req}>
              Reject
            </button>
          </div>
        </div>
