...
const WorkerRequests = () => {
  ...
  useEffect(() => {
    if (!workerinfo) {
      ...
      const fetch_workers_request=async()=>{
        try{
        const res=await axios.get(`http://localhost:8000/yash-services/services/get-hire-req/${workerinfo._id}`)
  ...
  <div className="bg-green-300 p-7 rounded-2xl flex flex-row gap-10 text-center justify-center " key={index}>
    <h1 className="font-mono text-center text-2xl">{item.fullname}</h1>
    ...
    <button className="bg-blue-500 text-white p-3 rounded-3xl text-center font-mono hover:cursor-pointer hover:opacity-75">View Request</button>
          ...
          :(<div className="mx-auto">
            <h1 className="text-center text-red-500 font-mono text-2xl">No Requests</h1>
          </div>)
        }
